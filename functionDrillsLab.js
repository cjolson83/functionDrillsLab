...
const printName = pName =>  console.log(pName)
printName('Cameron')
...
const compareNums = (x, y) => {if (x > y) {return(x)} else {return(y)}}

console.log(compareNums(12, 5))
...
const add = (x, y) => {x = +x; y = +y; return (x + y)}
console.log(add('3', '6'))
...
const nameCheck = pname => 
  {if (pname === 'Steven') {return "What is up Steven"}
  else if (pname === 'Bryan') {return "Hey Bryan!"}
  else {return `cool name, ${pname}`}
  }
console.log(nameCheck("Bob"))
...
const bigOrSmall = arr => {let answers = []
...
const theEliminator = (cont, los) => {for (let i = 0; i < cont.length; i++)
  {if (cont[i] === los) {cont.splice(i, 1)}}
  return(cont)}
...
const emailCheck = email => { email = email.trim()
  if (email.includes('@') === true) {return "email verified"}
  else {return "must provide a vail email address"}}

console.log(emailCheck("cj@gmail     "))
...
const frogsForGold = gold => {return gold/3}
totalFrogs = frogsForGold(17)
totalFrogs = Math.trunc(totalFrogs)
